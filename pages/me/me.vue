<template>
  <view>
    ...
    <view class="orders">
      <view class="box">
        <view class="label" v-for="(row, index) in orderTypeLise" :key="row.name" hover-class="hover" @click="changePage('order/order', index)">
          <view class="icon">
...
export default {
  ...
  methods: {
    ...
    changePage(e, index) {
      ...
      //   });
      // }
      console.log(`/pages/${e}?id=${index}`); 
      uni.navigateTo({
        url: `/pages/${e}?id=${index}`
      });
    },
    //用户点击列表项
    toPage(list_i, li_i) {
